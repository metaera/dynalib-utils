...
void SHA256::update(const unsigned char* message, uint len) {
...
void SHA256::final(uint8_t* digest) {
    uint block_nb = (1u + ((SHA224_256_BLOCK_SIZE - 9u) < (m_len % SHA224_256_BLOCK_SIZE)));
    uint len_b    = (m_tot_len + m_len) << 3u;
    uint pm_len   = block_nb << 6u;
    memset(m_block + m_len, 0, pm_len - m_len);
    m_block[m_len] = 0x80;
    SHA2_UNPACK32(len_b, m_block + pm_len - 4u);
...
uint8_t* sha256(std::string input, uint8_t* digest) {
    return sha256((unsigned char*)input.c_str(), (uint)input.length(), digest);
}